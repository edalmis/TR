...
const OtherProfile = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="profile-Page svelte-11kq53h"><h1 class="svelte-11kq53h">That is * ${escape(username)} * Profile Bro !</h1> <div><img class="profile-pic svelte-11kq53h"${add_attribute("src", pictureLink, 0)} alt=": 🤖 👨🏻‍🌾 🍪 🤣 :"></div> <div><p class="svelte-11kq53h">Login : ${escape(login)}</p> <p class="svelte-11kq53h">Name : ${escape(username)}</p> <p class="svelte-11kq53h">Total Won: ${escape(win)} - ${escape(loose)} :Lost</p> <p class="svelte-11kq53h">Rank : ${escape(rank)}</p> <p class="svelte-11kq53h">Title: ${escape(title)}</p></div> ${`${`${``}`}`}</div> <div>${``}</div> <div>${`${``}`}</div> ${validate_component(InviteToPlayButton, "InviteToPlayButton").$$render($$result, {}, {}, {})} <div><h1 class="svelte-11kq53h" data-svelte-h="svelte-1ki9rmi">Game History</h1> ${games.length > 0 ? `${each(games, (game, i) => {
